...
def test_compose_declares_tracing_profile_services() -> None:
    ...
    assert "webhook-worker:" in compose_text
...
def test_prometheus_scrapes_webhook_dispatcher() -> None:
    ...
    assert "job_name: webhook-worker" in prometheus_text
    assert 'targets: ["webhook-worker:9500"]' in prometheus_text
    assert "Solution2WebhookWorkerUnavailable" in alerts_text
