...
async def run_loadtest(
    *,
    ...
    async with httpx.AsyncClient(base_url=base_url, timeout=15.0, limits=limits) as client:
        ...
        for key in users:
            await _topup(client, target_key=key, target_credits=credits_needed)

        # Sustained load: schedule requests at target RPS
        rnd = random.Random(seed)
        sem = asyncio.Semaphore(rps * 3)  # limit in-flight concurrency
        interval = 1.0 / rps

        async def fire(i: int) -> Result:
            async with sem:
                api_key = users[i % len(users)]
                x, y = rnd.randint(1, 100), rnd.randint(1, 100)
                return await _submit(client, api_key, x, y)
        ...
        # Poll a sample of accepted tasks to verify completion
        ...
        for r in sample:
            api_key = users[0]  # any key works for poll
            status = await _poll_until_terminal(client, r.task_id or "", api_key)
...
def _print_report(report: dict[str, Any]) -> None:
    print()
    print("=" * 60)
    print("  Sustained Load Test — Solution 0")
    print("=" * 60)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sustained load test for solution 0")
