...
DOCKER_LOCK_RUNTIME_CHECK=1 ./scripts/docker_lock_check.sh | tee "${artifact_dir}/docker-lock.log"
docker compose down -v --remove-orphans >/dev/null 2>&1 || true
docker compose up --build -d
./scripts/wait_ready.sh "http://localhost:8000" 240 2

./scripts/reset_state.sh
pytest tests/integration -m integration | tee "${artifact_dir}/integration.log"

./scripts/reset_state.sh
pytest tests/e2e -m e2e | tee "${artifact_dir}/e2e.log"

./scripts/reset_state.sh
pytest tests/fault -m fault | tee "${artifact_dir}/fault.log"
