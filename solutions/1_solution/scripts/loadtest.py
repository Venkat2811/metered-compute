...
Separate from make prove and load_harness.py (burst profiles).
This tests sustained throughput at a target rate.

Usage:
    python scripts/loadtest.py
    python scripts/loadtest.py --rps 100 --duration 30
    python scripts/loadtest.py --rps 200 --users 20 --duration 60
"""
...
async def _get_token(client: httpx.AsyncClient, api_key: str, scope: str | None = None) -> str:
    """Exchange API key for JWT."""
    payload: dict[str, str] = {"api_key": api_key}
    if scope:
        payload["scope"] = scope
    resp = await client.post(V1_OAUTH_TOKEN_PATH, json=payload)
    assert resp.status_code == 200, f"token exchange failed: {resp.text}"
    token = str(resp.json().get("access_token", ""))
    assert token.count(".") == 2, "token is not JWT-like"
    return token
...
async def _topup(
    ...
    *,
    ...
    """Set user balance to target_credits via admin delta."""
    probe = await client.post(
        V1_ADMIN_CREDITS_PATH,
        headers={"Authorization": f"Bearer {admin_token}"},
        json={"api_key": target_key, "delta": 0, "reason": "loadtest_probe"},
    )
    assert probe.status_code == 200, f"admin probe failed: {probe.text}"
    ...
    resp = await client.post(
        V1_ADMIN_CREDITS_PATH,
        headers={"Authorization": f"Bearer {admin_token}"},
        json={"api_key": target_key, "delta": delta, "reason": "loadtest_set"},
    )
    assert resp.status_code == 200, f"admin set failed: {resp.text}"
...
async def run_loadtest(
    *,
    ...
    api_keys = [DEFAULT_USER1_API_KEY, DEFAULT_USER2_API_KEY]
    total = rps * duration
    credits_needed = (total // len(api_keys) + 1) * DEFAULT_TASK_COST * 2
    ...
    async with httpx.AsyncClient(base_url=base_url, timeout=15.0, limits=limits) as client:
        ...
        user_tokens = []
        for key in api_keys:
            token = await _get_token(client, key)
            user_tokens.append(token)

        # Top up credits generously
        for key in api_keys:
            await _topup(
                client, admin_token=admin_token, target_key=key, target_credits=credits_needed
            )
        ...
        async def fire(i: int) -> Result:
            async with sem:
                ...
                return await _submit(client, token, x, y)
        ...
        for r in sample:
            status = await _poll_until_terminal(client, r.task_id or "", user_tokens[0])
...
def _print_report(report: dict[str, Any]) -> None:
    print()
    print("=" * 60)
    print("  Sustained Load Test — Solution 1")
    print("=" * 60)
    total = report["total_requests"]
    pct = report["accepted"] * 100 / total if total else 0
    print(
        f"  Target:      {report['target_rps']} RPS x {report['duration_seconds']}s"
        f" = {total} requests"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sustained load test for solution 1")
