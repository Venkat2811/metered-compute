...
from solution1.workers import webhook_dispatcher
...
async def test_main_async_runs_single_cycle_and_shuts_down(
    ...
    monkeypatch.setattr(
        "solution1.workers.webhook_dispatcher.asyncpg.create_pool",
        fake_create_pool,
    )
    monkeypatch.setattr(
        "solution1.workers.webhook_dispatcher.Redis.from_url",
        lambda *_args, **_kwargs: fake_redis,
    )
    monkeypatch.setattr(
        "solution1.workers.webhook_dispatcher.httpx.AsyncClient",
    ...
    monkeypatch.setattr("solution1.workers.webhook_dispatcher.asyncio.Event", _OneShotEvent)
    monkeypatch.setattr(
        "solution1.workers.webhook_dispatcher.asyncio.get_running_loop",
        lambda: _FakeLoop(),
    )
    monkeypatch.setattr(
        "solution1.workers.webhook_dispatcher.start_http_server",
        lambda _port: None,
    )
    monkeypatch.setattr(
        webhook_dispatcher,
