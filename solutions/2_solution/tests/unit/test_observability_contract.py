...
from solution2.observability import metrics as metrics_module
...
def test_prometheus_scrapes_reaper_metrics_target() -> None:
    ...
    assert "watchdog" in jobs

    watchdog_targets = jobs["watchdog"]["static_configs"][0]["targets"]
    assert "watchdog:9400" in watchdog_targets
