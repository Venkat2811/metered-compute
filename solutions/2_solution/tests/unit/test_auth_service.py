...
from solution2.services.auth import resolve_user_from_api_key, revoke_jti
...
async def test_resolve_user_cache_hit_skips_db(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("solution2.services.auth.fetch_user_by_api_key", fake_fetch_user_by_api_key)
...
async def test_schema_miss_metric_does_not_double_count_as_cache_miss(
    ...
    monkeypatch.setattr("solution2.services.auth.AUTH_CACHE_RESULTS_TOTAL", metrics)
    monkeypatch.setattr("solution2.services.auth.fetch_user_by_api_key", fake_fetch_user_by_api_key)
...
async def test_revoke_jti_dual_writes_redis_then_postgres(
    ...
    monkeypatch.setattr("solution2.services.auth.insert_revoked_jti", fake_insert_revoked_jti)
