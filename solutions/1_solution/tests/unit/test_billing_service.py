...
class _FakeRedis:
    def __init__(self) -> None:
        ...
        self.last_evalsha_args: tuple[object, ...] = ()
    ...
    async def evalsha(self, *args: object) -> str:
        self.evalsha_calls += 1
        self.last_evalsha_args = args
...
async def test_hydrate_credits_from_db_sets_cache(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "solution1.services.billing.fetch_user_credits_by_api_key",
...
async def test_run_admission_gate_parses_success_decision() -> None:
    ...
    decision, sha = await run_admission_gate(
        ...
        max_concurrent=3,
        stream_key="tasks:stream:test",
        task_ttl_seconds=1800,
        stream_payload={"x": 2, "y": 3},
        stream_maxlen=1000,
    )

    assert decision == AdmissionDecision(ok=True, reason="OK", existing_task_id=None)
    assert sha == "sha"
    assert fake_redis.last_evalsha_args[1] == 5
    assert fake_redis.last_evalsha_args[5] == "tasks:stream:test"
    assert str(fake_redis.last_evalsha_args[7]) == "10"
    assert str(fake_redis.last_evalsha_args[14]) == "1000"
    assert '"x": 2' in str(fake_redis.last_evalsha_args[11])
...
async def test_refund_and_decrement_active_marks_dirty_and_updates_sha(
    ...
    monkeypatch.setattr(
        "solution1.services.billing.decrement_active_counter",
