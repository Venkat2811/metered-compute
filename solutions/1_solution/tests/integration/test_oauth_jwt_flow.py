...
USER1_USER_ID = os.getenv(
    "INTEGRATION_USER1_USER_ID",
    os.getenv("OAUTH_USER1_USER_ID", "47b47338-5355-4edc-860b-846d71a2a75a"),
)
...
def _redis_cli(*args: str) -> str:
    result = _compose("exec", "-T", "redis", "redis-cli", "-n", "0", *args)
...
def _get_redis_balance(*, user_id: str) -> int:
    value = _redis_cli("GET", f"credits:{user_id}")
    assert value and value != "(nil)", f"missing redis balance for user_id={user_id}"
    return int(value)
...
def test_jwt_model_class_cost_multipliers(api_client: httpx.Client) -> None:
    ...
    before = _get_redis_balance(user_id=USER1_USER_ID)
    ...
    after_small = _get_redis_balance(user_id=USER1_USER_ID)
    ...
    after_large = _get_redis_balance(user_id=USER1_USER_ID)
