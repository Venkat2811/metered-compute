...
def _redis_balance(repo_root: Path, *, user_id: str) -> int:
    result = _compose(
        repo_root,
        "exec",
        "-T",
    ...
    _assert(result.returncode == 0, f"redis balance probe failed: {result.stderr}")
    raw = result.stdout.strip()
    _assert(raw and raw != "(nil)", f"redis balance key missing for user_id={user_id}")
    return int(raw)
...
def scenario_jwt_tier_model_stress(
    ...
    *,
    ...
    before = _redis_balance(repo_root, user_id=DEFAULT_USER1_OAUTH_USER_ID)
    ...
    after_small = _redis_balance(repo_root, user_id=DEFAULT_USER1_OAUTH_USER_ID)
    ...
    after_large = _redis_balance(repo_root, user_id=DEFAULT_USER1_OAUTH_USER_ID)
