"""FastAPI application assembly and shared runtime helpers for Solution 1."""
...
from solution1.api.admin_routes import register_admin_routes
from solution1.api.contracts import (
    AdminRoutesApp,
    SystemRoutesApp,
    TaskReadRoutesApp,
    TaskWriteRoutesApp,
...
from solution1.api.system_routes import register_system_routes
from solution1.api.task_read_routes import register_task_read_routes
from solution1.api.task_write_routes import register_task_write_routes
from solution1.api.webhook_routes import register_webhook_routes
...
from solution1.core.dependencies import DependencyHealthService, build_dependency_health_service
...
from solution1.services.billing import (
...
from solution1.utils.lua_scripts import ADMISSION_LUA, DECR_ACTIVE_CLAMP_LUA

logger = get_logger("solution1.api")

# Route modules receive this module object and resolve these symbols dynamically.
# Tests monkeypatch these names directly on `solution1.app`.
...
async def _check_worker_connectivity(
    *,
    ...
    """Probe stream worker liveness via Redis heartbeat key."""
    try:
        heartbeat_value = await asyncio.wait_for(
            redis_client.get(heartbeat_key),
            timeout=timeout_seconds,
        )
        return bool(heartbeat_value)
    except Exception:
        return False
...
def create_app(
    ...
    """Create the FastAPI application for Solution 1."""

    # Optional injection is preserved for testing, but runtime uses lifespan initialization.
    _ = settings
    _ = dependency_health

    app = FastAPI(title="mc-solution1-api", lifespan=_lifespan)
