# RFC-0005: Request Flow Diagrams
...
Compare: Sol 0 naive (PG-only) would be ~12 PG calls for the same lifecycle. Sol 5 replaces most PG calls with TB (billing) and Redis (cache), keeping PG for metadata only.
