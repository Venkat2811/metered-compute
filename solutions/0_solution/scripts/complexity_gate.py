...
OVERRIDES: dict[str, Threshold] = {
    ...
    "src/solution0/app.py::create_app": Threshold(max_complexity=110, max_lines=700),
    # Submit orchestration is intentionally explicit in task_write_routes.
    "src/solution0/api/task_write_routes.py::register_task_write_routes": Threshold(
        max_complexity=110, max_lines=700
    ),
    "src/solution0/api/task_write_routes.py::submit_task": Threshold(
        max_complexity=55, max_lines=320
    ),
    # Worker run_task orchestrates retries, transition guards, and compensation.
    # Keep visible and bounded instead of disabling the gate globally.
    "src/solution0/workers/worker_tasks.py::run_task": Threshold(max_complexity=25, max_lines=170),
}
