...
def _poll_until_terminal(
    ...
    *,
    ...
    for _ in range(max_attempts):
        ...
        if payload.get("status") in {"COMPLETED", "FAILED", "CANCELLED", "TIMEOUT", "EXPIRED"}:
            return payload
...
@pytest.mark.integration
def test_batch_submit_and_poll(api_client: httpx.Client) -> None:
    user_token = _oauth_token(client=api_client, api_key=USER1_KEY)
    submit = api_client.post(
        ...
        json={
            "tasks": [
                {"x": 2, "y": 3, "model_class": "small"},
                {"x": 10, "y": 5, "model_class": "medium"},
            ]
        },
    )
    ...
    first_terminal = _poll_until_terminal(api_client, task_id=task_ids[0], access_token=user_token)
    second_terminal = _poll_until_terminal(api_client, task_id=task_ids[1], access_token=user_token)
    assert first_terminal["status"] == "COMPLETED"
    assert second_terminal["status"] == "COMPLETED"


@pytest.mark.integration
def test_enterprise_sync_inline_returns_terminal_response(api_client: httpx.Client) -> None:
    ...
    submit = api_client.post(
        V1_TASK_SUBMIT_PATH,
        headers={"Authorization": f"Bearer {admin_token}", "Idempotency-Key": f"sync-{uuid4()}"},
        json={"x": 5, "y": 6, "mode": "sync", "model_class": "small"},
    )
    assert submit.status_code == 201, submit.text
    payload = submit.json()
    assert payload["status"] == "COMPLETED"
    assert payload["result"] == {"z": 11}
