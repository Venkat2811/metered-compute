# RFC-0005: Capacity Model

Parent: [RFC-0005 README](./README.md)
...
**Comparison to Sol 2**: Sol 2 PG carries ~5,600 peak writes/sec (including credit reservations, outbox events, inbox events, credit transactions, task commands + query projections). Sol 5 cuts PG writes by ~43% because all billing SQL is in TigerBeetle.
...
Compare Sol 2: ~8.7 TB (tasks + credit_reservations + credit_transactions + outbox + inbox). Sol 5 cuts PG storage by ~50%.
...
Compare Sol 0/1: ~14 GB (credit balances, task hashes, active counters, rate limits, stream data). Sol 5 cuts Redis memory by ~90% because billing state is in TB, not Redis.
...
| Phase               | Sol 0 (PG-only) | Sol 2 (CQRS+Outbox) | Sol 5 (TB+Restate) |
| ------------------- | ---------------- | -------------------- | ------------------- |
...
Sol 5 cuts PG calls by ~67% vs Sol 0 and ~60% vs Sol 2. The total call count is comparable to Sol 2, but the load is distributed across purpose-built systems (TB for billing, Redis for cache, Restate for orchestration) rather than concentrated on Postgres.
