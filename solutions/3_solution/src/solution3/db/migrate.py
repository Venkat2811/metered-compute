...
MIGRATION_ADVISORY_LOCK_ID = 4_257_003
...
def migration_template_values(settings: AppSettings | None = None) -> dict[str, str]:
    resolved_settings = settings or load_settings()
    return {
        "ADMIN_API_KEY": resolved_settings.admin_api_key,
        "ALICE_API_KEY": resolved_settings.alice_api_key,
        "BOB_API_KEY": resolved_settings.bob_api_key,
        "ADMIN_USER_ID": str(resolved_settings.oauth_admin_user_id),
        "TEST_USER1_USER_ID": str(resolved_settings.oauth_user1_user_id),
        "TEST_USER2_USER_ID": str(resolved_settings.oauth_user2_user_id),
        ...
        "REQUEST_MODE_VALUES_SQL": REQUEST_MODE_VALUES_SQL,
        "DEFAULT_MODEL_CLASS": DEFAULT_MODEL_CLASS,
        "MODEL_CLASS_VALUES_SQL": MODEL_CLASS_VALUES_SQL,
        "DEFAULT_TASK_STATUS": DEFAULT_TASK_STATUS,
        "TASK_STATUS_VALUES_SQL": TASK_STATUS_VALUES_SQL,
        "DEFAULT_BILLING_STATE": DEFAULT_BILLING_STATE,
        "BILLING_STATE_VALUES_SQL": BILLING_STATE_VALUES_SQL,
...
def load_migration_sql(path: Path, values: Mapping[str, str]) -> str:
    rendered = render_migration_sql(path.read_text(encoding="utf-8"), values).strip()
    if not rendered:
        raise ValueError(f"migration file is empty: {path}")
    if "{{" in rendered or "}}" in rendered:
        raise ValueError(f"unresolved migration placeholders in {path.name}")
    return rendered
...
async def apply_pending_migrations(connection: asyncpg.Connection, directory: Path) -> list[str]:
    ...
    for migration_file in ordered_migration_files(directory):
        ...
        sql = load_migration_sql(migration_file, template_values)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Apply SQL migrations for Solution 3")
    parser.add_argument(
        "--dsn",
        default=None,
