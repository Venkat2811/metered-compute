...
def _load_app_module() -> Any:
    module = cast(Any, importlib.import_module("solution1.app"))
...
@pytest.mark.asyncio
async def test_check_worker_connectivity_paths() -> None:
    app_module = _load_app_module()
    ...
    assert (
        await app_module._check_worker_connectivity(
            redis_client=cast(Any, healthy),
            heartbeat_key="workers:stream:last_seen",
        )
        is True
    )
    assert healthy.seen_key == "workers:stream:last_seen"

    assert (
        await app_module._check_worker_connectivity(
            redis_client=cast(Any, _BrokenRedis()),
            heartbeat_key="workers:stream:last_seen",
        )
        is False
    )
...
async def test_oauth_token_rate_limit_enforced() -> None:
    ...
    payload = OAuthTokenRequest(client_id="solution1-user1", client_secret="secret")
...
def test_jwks_factory_and_oauth_principal_mapping() -> None:
    ...
    assert (
        app_module._oauth_principal_for_client(client_id="solution1-admin", settings=settings).role
        == UserRole.ADMIN
    )
    assert (
        app_module._oauth_principal_for_client(client_id="solution1-user1", settings=settings).role
        == UserRole.USER
    )
    assert (
        app_module._oauth_principal_for_client(client_id="solution1-user2", settings=settings).role
        == UserRole.USER
    )
...
async def test_resolve_user_from_jwt_token_branch_coverage(
    ...
    monkeypatch.setattr(
        app_module.jwt,
        "decode",
        lambda *_a, **_k: {"client_id": "solution1-user1", "sub": "other-sub"},
    )
    assert await app_module.resolve_user_from_jwt_token(token="a.b.c", request=request) is None
    ...
    settings.hydra_expected_audience = "solution1-api"

    def _decode_with_aud(*_: object, **kwargs: object) -> dict[str, str]:
        assert kwargs["audience"] == "solution1-api"
...
def test_oauth_client_credential_resolution_branches() -> None:
    ...
    assert client_id == "solution1-admin"
    assert client_secret == "solution1-admin-secret"
