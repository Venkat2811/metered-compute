...
logger = get_logger("solution2.workers.webhook_dispatcher")
...
async def _pop_pending_event(runtime: DispatcherRuntime) -> str | None:
    ...
    try:
        item = await runtime.redis_client.blpop(
            _queue_key(runtime.settings),
            timeout=timeout_seconds,
        )
    except RedisTimeoutError:
        # Treat socket timeout as an empty poll cycle for the long-poll worker loop.
        return None
    if item is None:
        return None
    _queue, payload = item
...
async def _process_raw_event(
    ...
    *,
    ...
    with start_span(
        tracer_name="solution2.webhook_dispatcher",
...
async def main_async() -> None:
    ...
    redis_client = Redis.from_url(
        ...
        socket_timeout=max(
            ...
            float(settings.webhook_dispatcher_poll_timeout_seconds) + 0.5,
        ),
        socket_connect_timeout=settings.redis_socket_connect_timeout_seconds,
    )
    http_client = httpx.AsyncClient(timeout=_delivery_timeout_seconds(settings))
